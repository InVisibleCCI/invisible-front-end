...
export const LeafletMap = ({ events }) => {
  ...
  //     if (!store.address.latitude || !store.address.longitude) { return }
  //     return (
  //       <Marker key={store.id} icon={whichMarker(store.id) ? iconSelected : iconTicady} position={[store.address.latitude, store.address.longitude]}>
  //         <Tooltip className="tooltip-map">{store.name}</Tooltip>
  //       </Marker>
  //     )
  //   })
  // }
  ...
  return (
    <LeafletMapWrapper>
      {/*{stores && bounds &&*/}
      {/*  <Map bounds={bounds} center={position} zoom={14}>*/}
      {/*    <TileLayer*/}
      ...
      {/*  </Map>*/}
      {/*}*/}
      <MapContainer center={[48.5734053, 7.7521113]} zoom={13} scrollWheelZoom={false}>
        <TileLayer
